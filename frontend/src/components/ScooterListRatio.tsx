...
const ScooterListRatio = () => {
  const [scooterList, setScooterData] = useState(null);

  const { data, isLoading, error } = useQuery({
    queryKey: ["scooterQ", "sc"],
    queryFn: () =>
      fetch(`http://localhost:8080/api/scooters/leaderboard/ratio`).then(
        (res) => res.json()
      ),
  });
                  ...
                  <table className="table table-bordered table-hover">
                    <thead className="thead-light">
                      <tr>
                        ...
                        <th className="text-right">Claimed Range (km)</th>
                    ...
                    <tbody>
                      {scooterList.map((scooter) => (
                        <tr key={scooter.id}>
                          ...
                          <td className="text-right">
                            {parseFloat(scooter.claimedRange.toFixed(2))}
                          </td>
                          <td className="text-right">
                            {parseFloat(
                              (
                                scooter.realRange / scooter.claimedRange
                              ).toFixed(2)
                            )}
                          </td>
                        </tr>
...
export default ScooterListRatio;
